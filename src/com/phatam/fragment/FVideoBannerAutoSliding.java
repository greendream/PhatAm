...
public class FVideoBannerAutoSliding extends SherlockFragment implements
		OnTimerTickListeners, OnClickListener, OnPageChangeListener {

	public static String CLASS_PATH = FVideoBannerAutoSliding.class.toString();
	...
	private ArrayList<MVideoItem> mVideoItems = new ArrayList<MVideoItem>();
	...
	private void setVideoItems(ArrayList<MVideoItem> pVideoItems) {
		...
		try {
			...
			tvVideoArtist.setText(mVideoItems.get(vpImages.getCurrentItem()).getArtistName());

			// Create timer to control auto Sliding
			mTimeTicker = new TimeTicker(4000);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		layoutVideoInfo = (LinearLayout) rootView.findViewById(R.id.layoutVideoInfo);
	...
	private ArrayList<MVideoItem> getNewVideo() {
	...
	public void onPageSelected(int position) {
		try {
			tvVideoTitle.setText(mVideoItems.get(position).getVideoTitle());
			tvVideoArtist.setText(mVideoItems.get(position).getArtistName());
			layoutVideoInfo.startAnimation(AnimationUtils.loadAnimation(getActivity(), R.animator.push_up_in));
		} catch (Exception e) {
		}
	}
}
